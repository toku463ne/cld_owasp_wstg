...
1. 実行可能/悪性ファイル（Web シェル・EICAR テストウイルス・巨大ファイル・XXE 入り docx/svg）を上げて処理を確認
   > ⚠️ **負荷注意（手順1）**: 巨大ファイルや XXE 入りファイルのアップロードは処理側のメモリ/CPU/ディスクを圧迫しうる。サイズは控えめにし、検体は EICAR 等の無害なものを使う。
...
- **手順1**: 巨大ファイルや XXE 入りファイルのアップロードは処理側のメモリ/CPU/ディスクを圧迫しうる。サイズは控えめにし、検体は EICAR 等の無害なものを使う。
