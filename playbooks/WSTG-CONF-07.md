...
1. `curl -sD evidence/<活動フォルダ>/artifacts/root-headers.txt -o /dev/null https://target/` で応答ヘッダを保存し（この後の手順・WSTG-CLNT-09 でも使い回す）、`grep -i '^strict-transport-security:' evidence/<活動フォルダ>/artifacts/root-headers.txt || [ $? -eq 1 ]` で HSTS の有無・値を確認（何も出なければ HSTS なし＝fail 候補。grep の「一致なし」は失敗扱いにしない）
