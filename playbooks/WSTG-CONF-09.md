...
2. 手順1の `ls -la` 出力で、設定ファイル・秘密鍵・ログのパーミッションを見る。危険な例: その他ユーザ読取可（`----r--` の others r、例 `-rw-r--r--` の `.env`/`*.key`/`config.php`）、Web実行ユーザ（www-data等）が所有 or 書込可な設定ファイル（改ざん・Webshell設置につながる）。該当ファイルとモード（例 `644 config.php owner=www-data`）を artifacts に控える
3. アップロード保存先ディレクトリ（`uploads/` 等）に実行権が付いていないか見る: `ls -la` でディレクトリ/ファイルに `x`（例 `-rwxr-xr-x` の *.php）が無いか、Web からそのパスのスクリプトが実行されないか（`curl` で設置した無害ファイルが text で返るか実行されるか）。実行可能なら Webshell 設置リスクとして finding に
