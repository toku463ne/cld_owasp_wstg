...
1. 同一アカウントに誤ったパスワードで連続ログイン（Burp Intruder / `hydra`）し、ロックされる閾値を確認（少数から）
   > ⚠️ **負荷注意（手順1）**: これは意図的にロックさせるテスト。本番アカウントや共有アカウントでやると実利用者を締め出す。必ず専用のテストアカウントで、少数から段階的に。
...
- **手順1**: これは意図的にロックさせるテスト。本番アカウントや共有アカウントでやると実利用者を締め出す。必ず専用のテストアカウントで、少数から段階的に。
