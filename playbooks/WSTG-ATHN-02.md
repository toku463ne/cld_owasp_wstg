...
1. 既定資格情報（`admin/admin`・ベンダ既定）を管理画面・アプリログインで試す（まとめて試すなら `hydra` や Burp Intruder）
   > ⚠️ **負荷注意（手順1）**: 既定資格情報を hydra / Intruder でまとめて試すとアカウントロックを誘発する。まず1つずつ手で試し、ロック閾値を把握してから。専用のテストアカウントで。
...
- **手順1**: 既定資格情報を hydra / Intruder でまとめて試すとアカウントロックを誘発する。まず1つずつ手で試し、ロック閾値を把握してから。専用のテストアカウントで。
