...
1. 応答ヘッダを保存して中間装置の印を探す: `curl -sD evidence/<活動フォルダ>/artifacts/resp-headers.txt -o /dev/null https://target/ && { grep -iE '^(via|x-cache|x-served-by|server|set-cookie|x-forwarded-for|cf-ray|x-varnish|x-amz-cf-id):' evidence/<活動フォルダ>/artifacts/resp-headers.txt || [ $? -eq 1 ]; }`。ヒットしたヘッダ（`Via`/`X-Cache`=キャッシュ・LB、`cf-ray`=Cloudflare 等）から中間装置を推定
