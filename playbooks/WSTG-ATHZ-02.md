...
3. UI上は非表示/無効の機能（管理メニュー・ボタン・タブ）でも、その裏のエンドポイントを低権限セッションで直接叩くと実行できてしまわないか確認する。HTMLソース・JS・API定義から管理用URL/APIパスを拾い、Burp Repeaterで低権限Cookieのまま送る。通る＝サーバ側で認可しておらずクライアント任せ（＝保護不十分）
