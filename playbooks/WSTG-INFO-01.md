...
4. crt.sh（`https://crt.sh/?q=%25.target.co.jp`）と `subfinder -d target.co.jp -all -o evidence/<活動フォルダ>/artifacts/subfinder.txt` で公開・失念サブドメインを列挙（subfinder は API 主体で HTTPS_PROXY を見るためプロキシ配下でも動く。`-all` は無料ソースを総当り）
