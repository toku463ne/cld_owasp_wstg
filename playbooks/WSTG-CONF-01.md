...
3. `nikto -h https://target ${https_proxy:+-useproxy "$https_proxy"} -o evidence/<活動フォルダ>/artifacts/nikto.txt` と特定製品の既知脆弱性・既定資格情報を照合（nikto は環境変数のプロキシを見ないので、プロキシ経由で対象に出る環境では `-useproxy` を明示する。上の `${https_proxy:+...}` は https_proxy が設定されているときだけ付く＝直接届く環境ではそのまま直行する）
   > ⚠️ **負荷注意（手順3）**: nikto は既知パスへ大量のリクエストを送る非常に騒がしいスキャン。WAF/IPS の遮断や機器の高負荷を招きやすい。合意した時間内で、対象を1つずつ。
...
- **手順3**: nikto は既知パスへ大量のリクエストを送る非常に騒がしいスキャン。WAF/IPS の遮断や機器の高負荷を招きやすい。合意した時間内で、対象を1つずつ。
