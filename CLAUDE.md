...
- **`criteria.yaml` の手順は「簡潔な説明: \`コマンド\`。読み方を1文」の形で書く**。例:
  `"応答ヘッダと Cookie を保存して基盤を推定: \`curl …\`。Cookie 名・X-Powered-By から基盤を特定"`。
  record.html・カードの見出しはコマンドを抜いた説明（`step_desc`）なので、説明は目安 150 字・
  上限 300 字（selftest が検査）。**なぜその作りか（プロキシ事情・ツールの癖・除外条件・代替手段）は
  手順に書かず、その WSTG の `note` に書く**（カードの「補足」に出る）。
